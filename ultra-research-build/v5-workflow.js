...
async function verifyClaim(c) {
  ...
  // v5 HV1: 'corrected' requires the judge to name an external signal + replacement text — but is NOT gated on the
  // deterministic floor (the floor signals don't check factual/numeric accuracy; a judge-found primary source can correct a 'survived' claim).
...
phase('Finalize')
// v5 HV2: a 'killed' verdict is still a verdict — verification "ran" only if a load-bearing claim SURVIVED.
const survivedLB = verdicts.filter((v) => v.verdict !== 'killed')
...
const method = {
  ...
  verified: survivedLB.length, killedUnrecovered: killedLB, deferredUnverified: deferred.map((c) => c.id), initialDraftGatePassed: judge1.pass,
}
...
// v5 HV2/HV3: pass only on a CLEAN stop, with a load-bearing claim that SURVIVED, and NO unrecovered kills.
const cleanStop = stopReason === 'saturated' || stopReason === 'diminishing-returns'
const verificationRan = survivedLB.length > 0
...
if (!passed && typeof report === 'string') {
  const why = (judge2.pass === false) ? `failed exit gate: ${judge2.notes || ''}`
    : (!verificationRan ? 'no load-bearing claim survived verification'
...
const vaultSummary = `## ${Q}\n- Confidence: ${passed ? 'gated-pass' : 'PROVISIONAL'} · stop: ${stopReason} · tier: ${TIER}\n- Verified ${survivedLB.length} of ${verdicts.length} load-bearing claims (killed/unrecovered: ${killedLB.length}); ${corpus.size} sources; gaps: ${openGaps.length}\n- Open: ${openGaps.join('; ') || 'none'}\n`
