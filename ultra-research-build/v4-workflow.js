// ultra_research_engine.js  (v4) — deterministic engine for the Ultra Research skill.
...
// normalize+validate the tier ONCE so a mis-cased/typo'd string can't bypass the gate.
...
// a failed factored CoVe is a STANDALONE flag/kill signal. hasCitedCounter is now a GENUINE
// refutation (red.contradicts), not the mere existence of an adversary-surfaced link (v4 MR3).
...
while (round < C.maxRounds) {
  ...
  const nc = exactNew.length
    ...
    : null
  // saturation off the enumerable novelIndices, clamped to the batch + deduped (v4 LR1)
...
else if (round >= C.maxRounds && !openGaps.length && stopReason === 'saturated') { stopReason = 'covered-at-cap' } // productive at cap but all subtopics covered (v4 LR2)
...
if (judge1.pass === false && round < C.maxRounds && openGaps.length) {
  ...
  // v4 HR2: gap-fill added evidence — refresh completeness so a now-closed gap isn't reported as a stale provisional gap.
  ...
  if (gc) {
    ...
    if (!openGaps.length && stopReason === 'saturated-with-gaps') stopReason = 'saturated' // PROMOTE only; never relax a real gap
  }
}
...
// v4 MR1: collect ALL snippets per URL (one URL legitimately backs several distinct claims).
...
async function verifyClaim(c) {
  ...
  // snippet-faithfulness against ALL snippets captured for this URL (v4 MR1); neutral if none
  const snips = (snippetsByUrl.get(c.source_url) || []).filter(Boolean)
  const faith = snips.length
    ? ((await safeAgent(
        `Do ANY of these source snippets actually support the claim, or is the claim an overreach / misread of all of them? Claim: "${c.text}". Snippets: ${JSON.stringify(snips.slice(0, 4).map((s) => s.slice(0, 400)))}. Return {faithful:boolean, why}.`,
  ...
  // v4 MR3: a counter only counts if RED says it GENUINELY contradicts — not mere existence of a link.
  ...
  // v4 MR4: 'corrected' requires a REAL deterministic signal (floor already >= flagged) + the signal+text, not asserted prose alone.
  if (verdict === 'corrected' && (RANK[floor] < RANK['flagged-uncertain'] || !(j.externalSignal && j.correctedText))) verdict = floor
...
// v4 MR2: prioritize load-bearing-cited (incl. re-sourced) records so the compress cap can't drop them.
...
const method = {
  ...
  verified: verdicts.length, deferredUnverified: deferred.map((c) => c.id), draftGatePassed: judge1.pass, // v4 HR1: count actual verdicts
}
...
// v4 HR1: passed requires a CLEAN stop AND that verification ACTUALLY ran (verdicts.length>0), fail-closed.
const cleanStop = stopReason === 'saturated' || stopReason === 'diminishing-returns' || stopReason === 'covered-at-cap'
const verificationRan = verdicts.length > 0
const passed = (judge2.pass !== false) && cleanStop && verificationRan
if (!passed && typeof report === 'string') {
  const why = (judge2.pass === false) ? `failed exit gate: ${judge2.notes || ''}` : (!verificationRan ? 'no claim-level verification ran' : `stop reason: ${stopReason} (${openGaps.length} open gap${openGaps.length === 1 ? '' : 's'})`)
  report = `> [!warning] PROVISIONAL — ${why}. Treat flagged findings as unverified; see Open questions & gaps.\n\n` + report
}
const vaultSummary = `## ${Q}\n- Confidence: ${passed ? 'gated-pass' : 'PROVISIONAL'} · stop: ${stopReason} · tier: ${TIER}\n- Verified ${verdicts.length} load-bearing claims; ${corpus.size} sources; gaps: ${openGaps.length}\n- Open: ${openGaps.join('; ') || 'none'}\n`
