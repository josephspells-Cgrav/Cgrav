...
C:/Users/josep/.claude/skills/orchestrator/SKILL.md

## Delta note (CORRECTED RE-BATON)
The prior re-baton was pointed at a STALE PRE-EDIT DRAFT by author error; its F1-F4/F6/F7/F10 were against bytes that no longer exist. THIS brief points at the INSTALLED file, which already carries: always-ask modes, admissible-skips-are-exactly-two, builder-owns-merge-conflicts, SHIP=one-round-no-new-majors, rollback-first, dead-lane disk resume, WO amendment protocol, six NEVERs, judge-packet + fresh-agent rule, untrusted-diff warning, canonical-WO-outside-worktree, activation vocabulary tightened. Audit THESE bytes.
